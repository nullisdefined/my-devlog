...
function setupCodeBlockButtons(
  ...
  const expandButtons = document.querySelectorAll(
    ".mermaid-container button[aria-label='다이어그램 확대']",
  );
  const codeButtons = document.querySelectorAll(
    ".mermaid-container button[aria-label='코드로 보기']",
  );
...
export function PostContentEnhancer() {
  ...
  const handleToggleView = useCallback(
    async (button: Element, view: "code" | "diagram") => {
      const container = button.closest(
        '[data-language="mermaid"], .mermaid-container',
      );
      if (!container) return;
  ...
  // 코드 -> 다이어그램 변환 함수
  const convertToMermaidDiagram = useCallback(
    async (container: HTMLElement, mermaidCode: string) => {
      try {
  ...
  const convertToCodeBlock = useCallback(
    (container: HTMLElement, mermaidCode: string) => {
      // 새 코드 블록 생성
      const pre = document.createElement("pre");
      pre.className =
        "relative group rounded-lg overflow-hidden border-l-4 border-l-blue-500 my-6";
  ...
  const applyImageStyles = useCallback(() => {
    ...
    images.forEach((img) => {
      ...
      // 기존 인라인 스타일 보존하면서 추가 스타일만 적용
      const existingCursor = imageElement.style.cursor;
      const existingTransition = imageElement.style.transition;
  ...
  }, [applyImageStyles, handleDiagramClick, handleToggleView]);
