...
category: "Git"
thumbnail: "https://nullisdefined.s3.ap-northeast-2.amazonaws.com//images/19b9ca1ab6310a720cb0ec27a521627f.png"
draft: true
---
Git은 파일과 디렉터리를 효율적으로 관리하기 위해 내부적으로 Blob, Tree, Commit이라는 세 가지 주요 객체를 사용한다. 이 글에서는 Git의 내부 동작 원리를 이해하기 위해 이 객체들이 무엇인지, 그리고 서로 어떻게 연결되어 있는지 살펴보겠다.

## Blob Object

Blob은 파일의 내용을 저장하는 객체이다. Git은 파일의 이름이나 메타데이터 대신, 파일의 내용만을 저장하며 이를 SHA-1 해시 값으로 관리한다.

### 특징
#### 1. 동일한 내용의 파일은 하나의 Blob 객체만 사용

Git은 파일 내용을 기준으로 Blob 객체를 생성하므로, 동일한 내용을 가진 파일으 ㄴ항상 동일한 Blob 객체를 공유한다.

예를 들어, "hello"라는 문자열을 파일로 저장하고 Blob 객체를 생성하면, 동일한 내용으로 Blob을 생성하는 다른 작업에서도 같은 SHA-1 해시 값을 얻게 된다. 이는 Git이 중복 데이터를 제거하고 저장 공간을 절약하는 원리다.
...
Git은 디렉터리의 내용이 변경될 때마다 새로운 Tree 객체를 생성한다. 따라서 Tree 객체는 특정 시점의 디렉터리 구조를 나타낸다.
...
Commit은 Git에서 프로젝트 상태(스냅샷)와 변경 이력을 기록하는 객체다. Commit은 특정 Tree를 가리키며, 부모 Commit과 연결되어 변경 이력을 추적한다.
...
Git 내부 구조를 이루는 세 객체의 역할과 상호작용을 이해할 수 있었다. Blob 객체는 파일의 실제 내용을 저장하고, 중복 데이터를 효율적으로 관리한다. Tree 객체는 디렉터리 구조를 표현하며, Blob과 하위 Tree 객체들을 조직화하고, Commit 객체는 프로젝트의 특정 시점 스냅샷을 Tree 객체로 참조하고, 변경 이력을 추적하는 역할을 한다.

또 Git LFS를 통해 대용량 파일도 효과적으로 관리할 수 있음을 살펴봤다. 이는 Git 기본 객체 모델을 확장하여 ..